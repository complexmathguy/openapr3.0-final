 ...
 * <li>Improving the available of ValuesMap related services in the case of a ValuesMap business related service failing.</li>
 * <li>Exposes a simpler, uniform ValuesMap interface to the business tier, making it easy for clients to consume a simple Java object.</li>
 * <li>Hides the communication protocol that may be required to fulfill ValuesMap business related services.</li>
 * </ol>
 * <p>
 * @author your_name_here
 */
public class ValuesMapBusinessDelegate 
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
		...
		try {
        	...
        	// -------------------------------------- 
			if ( command.getValuesMapId() == null )
				command.setValuesMapId( UUID.randomUUID() );
    		...
    		// ---------------------------------------
    		// issue the CreateValuesMapCommand - by convention the future return value for a create command
        	// that is handled by the constructor of an aggregate will return the UUID 
    		// ---------------------------------------
        	completableFuture = commandGateway.send( command );
        	
			LOGGER.log( Level.INFO, "return from Command Gateway for CreateValuesMapCommand of ValuesMap is " + command );
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
        ...
        try {
        	...
        	// --------------------------------------
        	CompletableFuture<ValuesMap> futureEntity = queryGateway.query(new FindValuesMapQuery( new LoadValuesMapFilter( summary.getValuesMapId() ) ), ResponseTypes.instanceOf(ValuesMap.class));
     ...
     */
    public List<ValuesMap> getAllValuesMap() 
    throws ProcessingException {
        List<ValuesMap> list = null;

        try {
        	CompletableFuture<List<ValuesMap>> futureList = queryGateway.query(new FindAllValuesMapQuery(), ResponseTypes.multipleInstancesOf(ValuesMap.class));
	...
	private ValuesMap load( UUID id ) throws ProcessingException {
		valuesMap = ValuesMapBusinessDelegate.getValuesMapInstance().getValuesMap( new ValuesMapFetchOneSummary(id) );	
		return valuesMap;
	}
