 ...
 * <li>Improving the available of Notifier related services in the case of a Notifier business related service failing.</li>
 * <li>Exposes a simpler, uniform Notifier interface to the business tier, making it easy for clients to consume a simple Java object.</li>
 * <li>Hides the communication protocol that may be required to fulfill Notifier business related services.</li>
 * </ol>
 * <p>
 * @author your_name_here
 */
public class NotifierBusinessDelegate 
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
		...
		try {
        	...
        	// -------------------------------------- 
			if ( command.getNotifierId() == null )
				command.setNotifierId( UUID.randomUUID() );
    		...
    		// ---------------------------------------
    		// issue the CreateNotifierCommand - by convention the future return value for a create command
        	// that is handled by the constructor of an aggregate will return the UUID 
    		// ---------------------------------------
        	completableFuture = commandGateway.send( command );
        	
			LOGGER.log( Level.INFO, "return from Command Gateway for CreateNotifierCommand of Notifier is " + command );
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
        ...
        try {
        	...
        	// --------------------------------------
        	CompletableFuture<Notifier> futureEntity = queryGateway.query(new FindNotifierQuery( new LoadNotifierFilter( summary.getNotifierId() ) ), ResponseTypes.instanceOf(Notifier.class));
     ...
     */
    public List<Notifier> getAllNotifier() 
    throws ProcessingException {
        List<Notifier> list = null;

        try {
        	CompletableFuture<List<Notifier>> futureList = queryGateway.query(new FindAllNotifierQuery(), ResponseTypes.multipleInstancesOf(Notifier.class));
	...
	private Notifier load( UUID id ) throws ProcessingException {
		notifier = NotifierBusinessDelegate.getNotifierInstance().getNotifier( new NotifierFetchOneSummary(id) );	
		return notifier;
	}
