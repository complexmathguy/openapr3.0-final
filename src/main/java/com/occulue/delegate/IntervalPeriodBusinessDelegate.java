 ...
 * <li>Improving the available of IntervalPeriod related services in the case of a IntervalPeriod business related service failing.</li>
 * <li>Exposes a simpler, uniform IntervalPeriod interface to the business tier, making it easy for clients to consume a simple Java object.</li>
 * <li>Hides the communication protocol that may be required to fulfill IntervalPeriod business related services.</li>
 * </ol>
 * <p>
 * @author your_name_here
 */
public class IntervalPeriodBusinessDelegate 
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
		...
		try {
        	...
        	// -------------------------------------- 
			if ( command.getIntervalPeriodId() == null )
				command.setIntervalPeriodId( UUID.randomUUID() );
    		...
    		// ---------------------------------------
    		// issue the CreateIntervalPeriodCommand - by convention the future return value for a create command
        	// that is handled by the constructor of an aggregate will return the UUID 
    		// ---------------------------------------
        	completableFuture = commandGateway.send( command );
        	
			LOGGER.log( Level.INFO, "return from Command Gateway for CreateIntervalPeriodCommand of IntervalPeriod is " + command );
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {	
        ...
        try {  
            ...
            final String errMsg = "Unable to delete IntervalPeriod using Id = "  + command.getIntervalPeriodId();
            LOGGER.log( Level.WARNING, errMsg, exc );
            throw new ProcessingException( errMsg, exc );
        }
        finally {
    ...
    throws ProcessingException, IllegalArgumentException {
        ...
        try {
        	...
        	// --------------------------------------
        	CompletableFuture<IntervalPeriod> futureEntity = queryGateway.query(new FindIntervalPeriodQuery( new LoadIntervalPeriodFilter( summary.getIntervalPeriodId() ) ), ResponseTypes.instanceOf(IntervalPeriod.class));
        	
        	entity = futureEntity.get();
        }
        catch( Exception exc ) {
            final String errMsg = "Unable to locate IntervalPeriod with id " + summary.getIntervalPeriodId();
            LOGGER.log( Level.WARNING, errMsg, exc );
            throw new ProcessingException( errMsg, exc );
        }
        finally {
     ...
     */
    public List<IntervalPeriod> getAllIntervalPeriod() 
    throws ProcessingException {
        List<IntervalPeriod> list = null;

        try {
        	CompletableFuture<List<IntervalPeriod>> futureList = queryGateway.query(new FindAllIntervalPeriodQuery(), ResponseTypes.multipleInstancesOf(IntervalPeriod.class));
	...
	private IntervalPeriod load( UUID id ) throws ProcessingException {
		intervalPeriod = IntervalPeriodBusinessDelegate.getIntervalPeriodInstance().getIntervalPeriod( new IntervalPeriodFetchOneSummary(id) );	
		return intervalPeriod;
	}
...
//************************************************************************
    ...
    private static final Logger LOGGER 			= Logger.getLogger(IntervalPeriodBusinessDelegate.class.getName());
