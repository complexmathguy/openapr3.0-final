...
public class ReportPayloadDescriptorTest{
	...
	protected void jumpStart() throws Throwable {
		...
		reportPayloadDescriptorId = ReportPayloadDescriptorBusinessDelegate.getReportPayloadDescriptorInstance()
	...
	protected void setUpQuerySubscriptions() throws Throwable {
		...
		try {            
			subscriber.reportPayloadDescriptorSubscribe().updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetAll update received for ReportPayloadDescriptor : " + successValue.getReportPayloadDescriptorId());
							  if (successValue.getReportPayloadDescriptorId().equals(reportPayloadDescriptorId)) {
								  if (testingStep.equals("create")) {
									  testingStep = "update";
									  update();
								  } else if (testingStep.equals("delete")) {
									  testingStep = "complete";
									  state( getAll().size() == sizeOfReportPayloadDescriptorList - 1 , "value not deleted from list");
									  LOGGER.info("**********************************************************");
			...
			subscriber.reportPayloadDescriptorSubscribe( reportPayloadDescriptorId ).updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetOne update received for ReportPayloadDescriptor : " + successValue.getReportPayloadDescriptorId() + " in step " + testingStep);
							  testingStep = "delete";
					  ...
					  error -> LOGGER.warning(error.getMessage()),
					  () -> LOGGER.info("Subscription on reportPayloadDescriptor for reportPayloadDescriptorId consumed")
	...
	protected ReportPayloadDescriptor read() throws Throwable {
		...
		StringBuilder msg = new StringBuilder( "-- Failed to read ReportPayloadDescriptor with primary key" );
		msg.append( reportPayloadDescriptorId );
		
		ReportPayloadDescriptorFetchOneSummary fetchOneSummary = new ReportPayloadDescriptorFetchOneSummary( reportPayloadDescriptorId );

		try {
			entity = ReportPayloadDescriptorBusinessDelegate.getReportPayloadDescriptorInstance().getReportPayloadDescriptor( fetchOneSummary );
	...
	protected void update() throws Throwable {
		...
		try {            
			...
			ReportPayloadDescriptorBusinessDelegate proxy = ReportPayloadDescriptorBusinessDelegate.getReportPayloadDescriptorInstance();  
			...
			LOGGER.warning( unexpectedErrorMsg );
			LOGGER.warning( msg.toString() + " : primarykey = " + reportPayloadDescriptorId + " : command -" +  command + " : " + e );
	...
	protected void delete() throws Throwable {
		...
		try{
		    entity = read(); 
			LOGGER.info( "-- Successfully read ReportPayloadDescriptor with id " + reportPayloadDescriptorId );            
		}
		catch ( Throwable e ) {
			LOGGER.warning( unexpectedErrorMsg );
			LOGGER.warning( "-- Failed to read ReportPayloadDescriptor with id " + reportPayloadDescriptorId );

			throw e;
		}

		try{
			ReportPayloadDescriptorBusinessDelegate.getReportPayloadDescriptorInstance().delete( new DeleteReportPayloadDescriptorCommand( entity.getReportPayloadDescriptorId() ) ).get();
			LOGGER.info( "-- Successfully deleted ReportPayloadDescriptor with id " + reportPayloadDescriptorId );            
		}
		catch ( Throwable e ) {
			LOGGER.warning( unexpectedErrorMsg );
			LOGGER.warning( "-- Failed to delete ReportPayloadDescriptor with id " + reportPayloadDescriptorId );
	...
	protected List<ReportPayloadDescriptor> getAll() throws Throwable {    
		...
		try {
			...
			collection = ReportPayloadDescriptorBusinessDelegate.getReportPayloadDescriptorInstance().getAllReportPayloadDescriptor();
			assertNotNull( collection, "An Empty collection of ReportPayloadDescriptor was incorrectly returned.");
			...
			while( iter.hasNext() ) {
				...
				assertNotNull( entity.getReportPayloadDescriptorId(), "-- entity in Collection has a null primary key" );        
	...
	protected CreateReportPayloadDescriptorCommand generateNewCommand() {
        CreateReportPayloadDescriptorCommand command = new CreateReportPayloadDescriptorCommand( null,  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  0.0F,  0,  ObjectType.values()[0] );
	...
	protected UpdateReportPayloadDescriptorCommand generateUpdateCommand() {
	        UpdateReportPayloadDescriptorCommand command = new UpdateReportPayloadDescriptorCommand( null,  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  0.0F,  0,  ObjectType.values()[0] );
