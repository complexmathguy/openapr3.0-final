...
/**
 * Projector for ReportPayloadDescriptor as outlined for the CQRS pattern.  All event handling and query handling related to ReportPayloadDescriptor are invoked here
 * and dispersed as an event to be handled elsewhere.
 * 
 * Commands are handled by ReportPayloadDescriptorAggregate
 * 
...
public class ReportPayloadDescriptorProjector extends ReportPayloadDescriptorEntityProjector {
	...
	// core constructor
	public ReportPayloadDescriptorProjector(ReportPayloadDescriptorRepository repository, QueryUpdateEmitter queryUpdateEmitter ) {
        super(repository);
    ...
    @SuppressWarnings("unused")
    @QueryHandler
    public List<ReportPayloadDescriptor> handle( FindAllReportPayloadDescriptorQuery query ) throws ProcessingException {
    	return findAll( query );
    }


	/**
	 * emit to subscription queries of type FindReportPayloadDescriptor, 
	...
	protected void emitFindReportPayloadDescriptor( ReportPayloadDescriptor entity ) {
	    ...
	    queryUpdateEmitter.emit(FindReportPayloadDescriptorQuery.class,
	                            query -> query.getFilter().getReportPayloadDescriptorId().equals(entity.getReportPayloadDescriptorId()),
	                            entity);
	}
	...
	protected void emitFindAllReportPayloadDescriptor( ReportPayloadDescriptor entity ) {
	    ...
	    queryUpdateEmitter.emit(FindAllReportPayloadDescriptorQuery.class,
	                            query -> true,
	                            entity);
	}
    ...
    private static final Logger LOGGER 	= Logger.getLogger(ReportPayloadDescriptorProjector.class.getName());
