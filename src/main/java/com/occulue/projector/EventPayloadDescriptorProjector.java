...
/**
 * Projector for EventPayloadDescriptor as outlined for the CQRS pattern.  All event handling and query handling related to EventPayloadDescriptor are invoked here
 * and dispersed as an event to be handled elsewhere.
 * 
 * Commands are handled by EventPayloadDescriptorAggregate
 * 
...
public class EventPayloadDescriptorProjector extends EventPayloadDescriptorEntityProjector {
	...
	// core constructor
	public EventPayloadDescriptorProjector(EventPayloadDescriptorRepository repository, QueryUpdateEmitter queryUpdateEmitter ) {
        super(repository);
    ...
    @SuppressWarnings("unused")
    @QueryHandler
    public List<EventPayloadDescriptor> handle( FindAllEventPayloadDescriptorQuery query ) throws ProcessingException {
    	return findAll( query );
    }


	/**
	 * emit to subscription queries of type FindEventPayloadDescriptor, 
	...
	protected void emitFindEventPayloadDescriptor( EventPayloadDescriptor entity ) {
	    ...
	    queryUpdateEmitter.emit(FindEventPayloadDescriptorQuery.class,
	                            query -> query.getFilter().getEventPayloadDescriptorId().equals(entity.getEventPayloadDescriptorId()),
	                            entity);
	}
	...
	protected void emitFindAllEventPayloadDescriptor( EventPayloadDescriptor entity ) {
	    ...
	    queryUpdateEmitter.emit(FindAllEventPayloadDescriptorQuery.class,
	                            query -> true,
	                            entity);
	}
    ...
    private static final Logger LOGGER 	= Logger.getLogger(EventPayloadDescriptorProjector.class.getName());
